...
use App\Imports\ImportBrokerLevel;
...
class ApiDataController extends Controller
{
    ...
    {
        if ($request->hasFile('scam_file')) {
            $scamBroker = $request->file('scam_file')->store('files');
        }
    
            if ($request->hasFile('level_file')) {
                $brokerLevel = $request->file('level_file')->store('files');
            }
            $excelFullPath = storage_path("app/{$scamBroker}");
            $excelPathLevel = storage_path("app/{$brokerLevel}");

        
        Excel::import(new ImportScamBroker, $excelFullPath);
        Excel::import(new ImportBrokerLevel, $excelPathLevel);

        return response()->json(['status'=>true, 'message' => 'Excel files imported successfully'], 200);
    }
    ...
    public function brokerPoint()
    {
        ...
        $brokerItems = Broker::query();
            if ($brokerName)
                $brokerItems->where('name', $brokerName);
                $brokerItems = $brokerItems->with('image')->whereHas('levels')->get();
        
        
        $brokerItems = $brokerItems->map(function ($item) {
                return [
                    'id' => $item->id,
                    'name' => $item->name,
                    'url' => $item->url,
        ...
        return response()->json(['status' => true, 'message' => 'Broker Points retrieved successfully', 'data' => $brokerItems], 200);
